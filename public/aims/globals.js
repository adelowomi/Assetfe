...
//AngularPath = "http://fe.asset.bz/aims/admin/index.html";
FileUploader = "http://picturehost.asset.bz/uploader.aspx";
//FileUploader = "http://localhost:56700/uploader.aspx";

baseUrl = "http://localhost:56700/"
//baseUrl = "http://be.asset.bz/"
//baseUrl = "http://api.asset.bz//"

PictureHostRoot = "http://picturehost.asset.bz/AIMS";
RootFolder = "AIMS";
GlobalDownloader = "http://localhost:56700/PDFdownloader.aspx";
//GlobalDownloader = "http://be.asset.bz/PDFdownloader.aspx";
PictureHostServer = "http://picturehost.asset.bz";

//PictureHostRoot = "http://localhost:3366/aims";
//Hostname = "http://localhost:3366";
//Hostname = "http://fe.asset.bz/aims"
Hostname = "http://www.asset.bz/aims";
ReportHostname = "http://www.asset.bz/aims";
PageExceptions = ["#/error-message", "#/child-menus", "#/userprofile", "#/pipe"]    
...
GlobalMessageAlert = function (message, type) {
	...
	var n = noty({
		layout: 'topRight',
		text: message, theme: 'relax', type: type, timeout: 10000, animation: {
...
AjaxFail = function (jpXHR, textStatus, thrownError) {
	...
	//HideBusyMode();
	if (jpXHR.status == 500) {
		GlobalMessageAlert('A critical application error occurred. Try again, and contact your system admin if this persists', 'error');
	}
};
		...
		if (Array.isArray(subject)) {
			...
			for (var i = 0; i < subject.length; i++) {
				for (var j = 0; j < keys.length; j++) {
					try {
						subject[i][keys[j]] =  subject[i][keys[j]].replace(/<script[^>]*?>.*?<\/script>/gi, '-').
							replace(/<[\/\!]*?[^<>]*?>/gi, '-').replace(/<style[^>]*?>.*?<\/style>/gi, '-').replace(/<![\s\S]*?--[ \t\n\r]*>/gi, '-');
					} catch (e) {}
				}
			}
		}
			...
			for (var j = 0; j < keys.length; j++) {
				try {
					subject[keys[j]] =  subject[keys[j]].replace(/<script[^>]*?>.*?<\/script>/gi, '-').
							replace(/<[\/\!]*?[^<>]*?>/gi, '-').replace(/<style[^>]*?>.*?<\/style>/gi, '-').replace(/<![\s\S]*?--[ \t\n\r]*>/gi, '-');
				} catch (e) { }
			}
		}
